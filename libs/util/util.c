    ...
    else
    {
        if (files_size > number_of_P)
        {
            ...
            for (i = 1; i <= number_of_P; i++)
            {
                ...
                ret_Ps[p_index].path_size = data_file_to_alloc;
...
p_input* old_get_P_input(data_file* files, int files_size, int number_of_P){
    if(files_size > number_of_P){
        int divide = files_size / number_of_P;
        int scarto = files_size % number_of_P;
        int data_file_to_alloc;
        ...
        int p_index = 0;
        p_input* ret_Ps = (p_input*) malloc (sizeof(p_input) * number_of_P);
        for(p_index = 0; p_index < number_of_P; p_index++) {
            for (i = 1; i <= divide; i+=data_file_to_alloc) {
                if (scarto != 0) {
                    data_file_to_alloc = divide + 1;
                    scarto--;
                } else {
        ...
        int i ;
        for(i = 0; i < files_size; i++){
            asprintf(ret_Ps[i].paths, "%s", files[i].path);
            ret_Ps[i].path_size = 1;
        }for( ; i < number_of_P; i++){
            ret_Ps[i].paths = NULL;
        }
